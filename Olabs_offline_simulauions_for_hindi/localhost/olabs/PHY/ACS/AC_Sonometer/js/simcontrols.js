...
var controlLabel,InsArray,opt1,opt2,supply_type_options,combo_2val=0;
var Q=0.05,Q1=0.5,color_arr;
...
function setAmplitude(){console.log("hit>?")
	...
	if(bridgeDist<(distance+30) && amplitude<62 && amplitude>2){console.log("hit>>>",bridgeDist,distance,vibrating,hited)
		
		if(bridgeDist<=(distance+2) && bridgeDist>=(distance-2)&& !vibrating && hited){
			startVibration(); console.log("hit>>>>>>>>");		
		}else{
...
window.onload = function() { 
	...
	controlLabel=[gt.gettext("Reset"),gt.gettext("Power On"),gt.gettext("Place paper rider"),gt.gettext("Power Off"),gt.gettext("Type of AC supply")];
	InsArray=[gt.gettext('Select environment'),gt.gettext("Add weighs(kg)"),gt.gettext("Select wire material"),gt.gettext("Diameter of the wire:"),gt.gettext('Frequency:'),gt.gettext('Weight hanger weight:'),1,0.05,0.5,100,55,gt.gettext('Position of bridge A:'),gt.gettext('Position of bridge B:')];	
	opt2=[gt.gettext("Copper"),gt.gettext("Steel"),gt.gettext("Nichrome"),gt.gettext("Constantan")];
	opt1=[gt.gettext("Earth (g=9.8 m/s&#178;)"),gt.gettext("Moon (g=1.6 m/s&#178;)"),gt.gettext("Uranus (g=8.69 m/s&#178;)"),gt.gettext("Saturn (g=10.44 m/s&#178;)"),gt.gettext("Jupiter (g=24.79 m/s&#178;)"),gt.gettext("Mars (g=3.711 m/s&#178;)"),gt.gettext("Venus (g=8.83 m/s&#178;)"),gt.gettext("Mercury (g=3.7 m/s&#178;)"),gt.gettext("Neptune (g=11.15 m/s&#178;)")];
	...
	$(document).ready(function() {
		...
		function addLabel(){//Function for adding label for all controls---	
			...
			$("#magnet_position_lbl").html(gt.gettext('Magnet position: '));
			$("#magnet_pos_val").html('50 cm');
			$("#magnet_lbl_min").html('10');
			$("#magnet_lbl_max").html('90');
			$("#bridge_A_txt").html(gt.gettext("Bridge A"));
		...
		$("#hitTfork").click(function(){
			...
			if(text==controlLabel[1]){
				hited=true;
				$("#light_on").show();
				if(supplyType==1){$("#switch").attr('src',simPath+'/images/switch_on.png')}
				bridgeDist=valueB-valueA;
				calculateDistance();				
				setAmplitude();
				...
				vibrating = false;
				$("#light_on").hide();
				if(supplyType==1){$("#switch").attr('src',simPath+'/images/switch_off.png')}
