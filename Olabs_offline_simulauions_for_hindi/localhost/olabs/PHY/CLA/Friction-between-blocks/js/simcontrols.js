...
window.onload = function(){ 
	...
	document.getElementById("expName").innerHTML=gt.gettext("Friction");
	...
	function BlockMove(){//(MOVE)
		...
		if(percent=="0" || winWidth<768){
			$("#ImgBlock" ).animate ({ left: '60.5%'}, {easing:"easeInExpo" ,duration: 3000, }); //Block step:function(){plot();}
		}
		else{
			$("#ImgBlock" ).animate ({ left: '66.8%'}, {easing:"easeInExpo" ,duration: 3000, }); //Block step:function(){plot();}
		}
		
		$('#twine').animate({ width: '5%' }, 3000,"easeInExpo");//Twine
		$("#weightBlock" ).animate({ top:'78%'   }, 3000,"easeInExpo" );//WeightBlock
		$('#twineTwo').animate({ height: '30%' },  3000,"easeInExpo");//Twine Two.
	...
	function changeBlockweight(val){
		...
		$('#BlockImage').css("width",46+MassOfBolock*0.9+"%");//changing the image width
	...
	$('#weightBar').css("height",7+(Comn.dickWeight) * 4+"%");//changing the image height (weight dick)
