// JavaScript Document
function dryheathelpfn(){//Colorless odourless gas 
	if($("#subDivision").find(':selected').val()==0){//Colorless odourless gas
		var dryheattrip1 = new Trip([
		{
			sel : $('#dragItem2_1'),
			position : 'w',
		...
		dryheattrip1.start(); 
		window.dryheattrip1 = dryheattrip1;
	}
	//Colorless gas with odour
	if($("#subDivision").find(':selected').val()==1){
		var dryheattrip2 = new Trip([
		{
			sel : $('#burnerhelpDiv'),
			position : 'n',
			content :dryHeatHelp[3],
		...
		dryheattrip2.start(); 
		window.dryheattrip2 = dryheattrip2;
	}
	if($("#subDivision").find(':selected').val()==2){
		//Colored gas
		var dryheattrip3 = new Trip([
		{
			sel : $('#dragItem2_5'),
			position : 'w',
		...
		dryheattrip3.start(); 
		window.dryheattrip3 = dryheattrip3;
	}
}	
	...
	$('#inferButton').bind('click',function(){
		...
		if($("#confirmation").find(':selected').val()==0){		
			if($("#preliminary").find(':selected').val()==1){
				if($("#subDivision").find(':selected').val()==0){//Colorless odourless gas
					if(sampleSalt[1]==sampleSalt[result] || sampleSalt[10]==sampleSalt[result]){//Lime water test	
						trip1 = new Trip([
						{
							sel : $('#dragItem2_1'),
							position : 'w',
