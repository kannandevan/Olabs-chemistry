var controlText,titrateItemstype,btnLabel,testtArray,toolTipMsgArray,restLbl,helpMsg;
window.onload = function init(){
	document.getElementById("expName").innerHTML=gt.gettext("Determination of Concentration/Molarity of KMnO<sub>4</sub> solution");
	...
	controlText=[gt.gettext('Drops rate:'),gt.gettext('Titrate:'),gt.gettext('Molarity of titrate, M1: '),gt.gettext('Volume of titrate, V1:'),gt.gettext('Result'),gt.gettext('Molarity of titrant, a '),'(V1 x M1 x n2) /(V2 x n1)',gt.gettext('No of moles of titrant, n2'),gt.gettext('No of moles of titrate, n1'),gt.gettext('Molar mass of titrant, b'),gt.gettext('Strength of given titrant, [a xb]'),'g/lit','M',gt.gettext('Show volume of titrant, V2'),'Dil. H<sub>2</sub>SO<sub>4</sub>',gt.gettext('Temperature: ')];
	titrateItemstype=[gt.gettext('Mohr<sup>&#39;</sup>s salt'),gt.gettext('Oxalic acid')];
	btnLabel=[gt.gettext("Start"),gt.gettext("Stop")];
	testtArray=[gt.gettext("Correct."),gt.gettext("Wrong! Try Again")];
	toolTipMsgArray=[gt.gettext("Burette"),gt.gettext("Titration flask"),gt.gettext("Clamp stand"),gt.gettext("Click on the nozzle or start button to start the titration."),gt.gettext("Click on the nozzle or stop button to stop the titration."),gt.gettext("Click on the reset button to reset the titration."),gt.gettext("Burner")];
	restLbl=gt.gettext("Reset");
	helpMsg=[gt.gettext('Select the titrate.'),gt.gettext('Increase/decrease the drops rate.'),gt.gettext('Change the molarity of the titrate.'),gt.gettext('Change the volume of the titrate.'),gt.gettext('Drag the test tube and </br> pour the  Sulphuric acid into </br> the conical flask containing </br> Mohr<sup>&#39;</sup>s salt.'),gt.gettext('Drag the conical flask and <br> place it below the burette.'),gt.gettext('Click on the nozzle or  </br> start button to start the titration.'),gt.gettext('Click on the checkbox to show volume of titrant.'),gt.gettext('Drag the conical flask and <br> place it above the burner.'),gt.gettext('Click on the burner <br> knob to start heating.'),gt.gettext('Drag the test tube and </br> pour the  Sulphuric acid into </br> the conical flask containing </br> Oxalic acid.')];
	//*--Ready function---------
	$(document).ready(function() {
		//*--Apply selectable false--------
		$('#bgCanvas,#buretteWater,#burette,#zoomArea,#drops,#buretteZoom,#solutionDiv,#volTitrant,#flask,#tooltp,#popup,#label_board,#burner,#burner_btn,#testtubeStand,#solutionBgDiv').mousedown(function(event){event.preventDefault();	});	
		//*--Array declaration----------
		
		var testtubeImgChange=['testtube_with_solution.png','testtube_no_solution.png'];
		var wrongorcorrectImg=["correct.png","wrong.png"];
		var reactions=["2KMnO<sub>4</sub>  +  8H<sub>2</sub>SO<sub>4</sub> +  10FeSO<sub>4</sub>(NH<sub>4</sub>)<sub>2</sub>SO<sub>4</sub>.6H<sub>2</sub>O "  +   String.fromCharCode(8594)   +   " K<sub>2</sub>SO<sub>4</sub>+2MnSO<sub>4</sub>+5Fe<sub>2</sub>(SO<sub>4</sub>)<sub>3</sub>+ 10(NH<sub>4</sub>)<sub>2</sub>SO<sub>4</sub>+68H<sub>2</sub>O","2KMnO<sub>4</sub> + 2H<sub>2</sub>SO<sub>4</sub> + <br> 5C<sub>2</sub>H<sub>2</sub>O<sub>4</sub>.2H<sub>2</sub>O  " +String.fromCharCode(8594) + " K<sub>2</sub>SO<sub>4</sub> + <br> 2MnSO<sub>4</sub> + 18H<sub>2</sub>O+ 10CO<sub>2</sub>"];
		...
		var conc=[1,2],molariystartValue=[2,1],numMoles=[10,5];
		//*--variable declaration------------------
		var noMolesTitrant=2,noMolesTitrate=numMoles[0];
		...
		var solnDivTop=bgTop+72,dropTop=solnDivTop-5,molarMassVal=158,vcolorTop=solnDivTop;
		var solnBgDivTop=bgTop+71;
		var molarMass=" "+molarMassVal+" g/mol";
		var dropCnt=0,TotalVolOfTitrate=0,dropCntTotal=401;
		var solnTop=$('#soln').position().top;
		var solnBgTop=$('#solnBG').position().top;
		var buretteWaterTop=$('#buretteWater').position().top;	
		var M1=1,V1=10,titrantVol=0.1,titrantStopVol=0,rotateEvt,rotateCnt=0,rotateCnt2=5,dropEvt;
		var speed=2000,waterInc=0.215,zooomwaterInc=.750,solnInc=0.07;
		var strength,keyFlag=false,animFlag=false,flameFlag=false;
		var flaming,tempChange,selectedTitrate=titrateItemstype[0];
		var startTemp=20,maxSpeed=3000,minSpeed=2500;
		...
		function temperatureChange(){
			...
			if(startTemp==90){	
				...
				dragItem('conicalflaskArea',bgTop+200,bgTop+270,bgLeft+200,bgLeft+320,236,272);
		...
		function startPour(){		
			dragItem('testtube',bgTop+170,bgTop+250,bgLeft+130,bgLeft+200,236,272);		
		}	
		//*--Function to start drag events---------
		function dragItem(sampleid,minTop,maxTop,minLeft,maxLeft,topPos,leftPos){
			//*-----Start drag event ----------------
			$('#'+sampleid).draggable({//stop the drag event	
				stop : function(event, ui) {
					if (((((this.offsetTop>=minTop)&& (this.offsetLeft>=minLeft)) && ((this.offsetTop<=maxTop)&&(this.offsetLeft<=maxLeft))) )){					
						 if(sampleid==dragObj[0]){ // Testtube 
							 if(animFlag==false){
								$("#testtubeImg").attr('src',simPath+"images/"+testtubeImgChange[1]);
								$("#testtube").draggable({disabled:true});
								$("#testtube").css({top:bgTop+179+'px',left:bgLeft+148+'px'});
								$("#testtube").rotate(90);
								...
								$('#solutionDiv').animate({height:solnHeight+'px',top:solnDivTop+'px' });
								...
								$('#solutionDiv2').animate({		
									height:solnHeight+'px',top:solnDivTop+'px'
								},500,function(){
								$("#solnPour,#solnPourStart").animate({
									opacity:'0'						
										...
										$("#testtube").rotate(0);
										$("#testtube").css({top:bgTop+100+'px',left:bgLeft+100+'px'});
										$("#testtube").animate({top:bgTop+223+'px',left:bgLeft+75+'px'});	
										animFlag=true;					
										dragConicalFlask();
									});						
								});
		...
		function dragConicalFlask(){
			$("#conicalflaskArea").draggable({disabled:false});
			$("#conicalflaskArea").css({cursor:'pointer'});	
			if(selectedTitrate==titrateItemstype[0]){ // Mohr's Salt
				dragItem('conicalflaskArea',bgTop+200,bgTop+270,bgLeft+200,bgLeft+320,236,272);
			}else {	// Oxalic acid								
				dragItem('conicalflaskArea',bgTop+70,bgTop+180,bgLeft+350,bgLeft+500,158,425);
			}
		}
		...
		$("#start,#onoffCommon").click(function(){	
			...
			if(	$("#start").val()==btnLabel[0]){
				...
				$('#molTitrant,#molesTitrate,#molesTitrant,#strengthval,#displayResult').removeAttr("disabled");
			}else{
				stopRotation();				
			}
		});
		...
		function dropFn(){
			...
			solnVioletWidth=10;
			solnVioletleft=293;
			$("#violetDiv").css({width:solnVioletWidth+'px',left:solnVioletleft+'px'});		
			$('#drops').animate({			
				top:dropTop+'px'
			},function(){						
				$('#drops').css({
		...
		function incrementSoln(){
			...
			$('#solutionDiv').css({height:solnHeight+'px',top:solnDivTop+'px'});
			$('#soln').css({top:solnTop+'px'});
			$('#solutionBgDiv2').css({top:solnBgDivTop+'px'});
			$('#solnBG2').css({top:solnBgTop+'px'});
			$('#solutionDiv2').css({height:solnHeight+'px',top:solnDivTop+'px'});
		...
		function changeEvent(){
			...
			$('#molTitrant').val("");
			$('#strengthval').val("");
			waterInc=0.215,zooomwaterInc=.750,solnInc=0.07;	
			solnDivTop=bgTop+72;
		...
		function toolTipMsgChange(){		
			...
			showToolTip('flask',toolTipMsgArray[1],'leftMiddle','rightMiddle',130,2,15+"px",'rightMiddle');
			showToolTip('stand',toolTipMsgArray[2],'rightMiddle','leftMiddle',130,2,15+"px",'leftMiddle');
			showToolTip('burner',toolTipMsgArray[6],'topBottom','bottomMiddle',110,2,15+"px",'rightMiddle');
