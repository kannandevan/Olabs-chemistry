//*--Ready function---------
var controlLabel,InsArray,num_array,Q=1,Q1=89,opacity_val=0,opacity_val1=1,equat_val,equat_val1,pour_topval;
...
var gt=$;
	window.onload = function() { 
		document.getElementById("expName").innerHTML=gt.gettext("Kinetic Study on the Reaction between Potassium iodate and Sodium sulphite");
		controlLabel=[gt.gettext("Reset"),gt.gettext('Volume of KIO<sub>3</sub> :'),gt.gettext('Volume of water :'),gt.gettext('Volume of H<sub>2</sub>SO<sub>4</sub> : 10 ml'),gt.gettext('Volume of Starch : 5 ml'),gt.gettext('Volume of Na<sub>2</sub>SO<sub>3</sub> : 10 ml')];
		num_array=[1,10,80,89];
		InsArray=[gt.gettext('START'),gt.gettext("STOP")];
		helprarr=[gt.gettext("Select the volume of <br>potassium iodate solution."),gt.gettext("Drag the potassium iodate<br> solution to the measuring jar."),gt.gettext("Drag the measuring jar to the <br>conical flask to pour the solution."),gt.gettext('Drag the sulphuric acid <br>to the measuring jar.'),gt.gettext("Drag the water to<br> the measuring jar."),gt.gettext("Drag the starch solution<br> to the measuring jar."),gt.gettext("Drag the sodium sulphite<br> solution to the measuring jar.")];
		label_content=[gt.gettext("1M H<sub>2</sub>SO<sub>4</sub>"),gt.gettext("Water"),gt.gettext("Starch"),gt.gettext('0.005M Sodium sulphite'),gt.gettext("0.01M KIO<sub>3</sub> ")];
		...
		drag_mate=['conical_flask','H2SO4_bottle','water_beaker','starch_bottle','sulphide_bottle'];
		waterob_arr=['conical_water','H2SO4_water','water_water','starch_water','sulphide_water']
		sample_pos=[[102,34,34,34,33],[71,84,211,308,408]];
		sample_tiltpos=[[176,188,151,190,190],[437,442,433,445,444]];
		min_maxarr=[[90,300,25,514]];
		water_originalpos=[[18,0,0,2,2],[-18,0,0,0,0]]
		waterpos=[22,13,33,13,13]
		$(document).ready(function() {	
			...
			function calculation(){	//calculationof time and result
				...
				time_calc=Math.round(parseFloat((179.2*parseFloat(1/Q))+parseFloat(-3.667)));	
				m = s = 0;
				stop();
				start();
